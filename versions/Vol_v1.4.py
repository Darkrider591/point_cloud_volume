...
from tkinter import filedialog, messagebox  # Импортируем модуль messagebox
...
def calculate_volume(upper_points, lower_points):
    """Расчет объема между поверхностями с учетом пересечений и ограничений по наименьшей площади площади"""
    try:
...
def main():
    ...
    if result is not None:
        show_results(result)  # Здесь вызываем функцию для показа результатов
    else:
