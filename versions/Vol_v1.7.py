...
def visualize_surfaces_gui(fig, upper_points, lower_points, result=None):
    ...
    # Отрисовываем все точки без прореживания
    ax.scatter(upper_points[:, 0], upper_points[:, 1], upper_points[:, 2],
               color='blue', label='Верхняя поверхность', s=5, alpha=0.6)
    ax.scatter(lower_points[:, 0], lower_points[:, 1], lower_points[:, 2],
               color='green', label='Нижняя поверхность', s=5, alpha=0.6)

    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.legend()
    ax.set_title("Поверхности")
    ...
    if result:
        ...
        ax.text2D(0.02, 0.98, '\n'.join(text_lines), transform=ax.transAxes, fontsize=9, va='top', bbox=props)
...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Button(frame_top, text="Выбрать верхнюю поверхность", command=self.load_upper).pack(side=tk.LEFT, padx=5)
        ttk.Button(frame_top, text="Выбрать нижнюю поверхность", command=self.load_lower).pack(side=tk.LEFT, padx=5)
        ttk.Button(frame_top, text="Рассчитать и построить график", command=self.calculate_and_plot).pack(side=tk.LEFT, padx=5)
        ttk.Button(frame_top, text="Сохранить результаты", command=self.save_results).pack(side=tk.LEFT, padx=5)
    ...
    def calculate_and_plot(self):
        ...
        # Визуализация
        visualize_surfaces_gui(self.fig, self.upper_points, self.lower_points, self.result)
        self.canvas.draw()
