...
# --- Расчёт объёма между поверхностями с интерполяцией ---
def calculate_volume(upper_points, lower_points):
    try:
        ...
        # Создаем KDTree для быстрого поиска ближайших точек на нижней поверхности
        ...
        for simplex in tri_upper.simplices:
            ...
            if Polygon(poly_upper).contains(Point(*centroid)):
                ...
                # Билинейная интерполяция на нижней поверхности
                dists, idxs = kd_lower.query(centroid, k=3)
                weights = 1 / (dists + 1e-8)  # Не нулевые веса
...
# --- Визуализация с цветовым отображением разницы высот ---
def visualize_surfaces_gui(fig, upper_points, lower_points, result=None):
    ...
    # Найдем среднюю разницу высот для цветовой шкалы
    all_z_diff = []
    kd_lower = KDTree(lower_points[:, :2])
    ...
    for simplex in tri_upper.simplices:
        ...
        z_upper = (A[2] + B[2] + C[2]) / 3
        height_diff = z_upper - z_lower
        all_z_diff.append(height_diff)

    all_z_diff = np.array(all_z_diff)
    norm = plt.Normalize(vmin=np.min(all_z_diff), vmax=np.max(all_z_diff))
    ...
    for simplex in tri_upper.simplices:
        ...
        color = cmap(norm(height_diff))
        ax.plot_trisurf([A[0], B[0], C[0]], [A[1], B[1], C[1]],
                        [z_upper, z_upper, z_upper],
                        color=color, alpha=0.6, linewidth=0.5, edgecolor='black')

    ax.scatter(lower_points[:, 0], lower_points[:, 1], lower_points[:, 2],
               c='gray', label='Нижняя поверхность', s=2, alpha=0.5)
    ...
    cbar.set_label('Разница высот (м)')
    ...
    if result:
        ...
        props = dict(facecolor='white', edgecolor='lightgray', alpha=0.9)
...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Button(frame_top, text="Экспортировать в Excel",
    ...
    def animate_plot(self):
        ...
        self.anim = FuncAnimation(self.fig, update, frames=360, interval=50, blit=False, repeat=True)
        self.canvas.draw()
