...
def calculate_volume(upper_points, lower_points):
    ...
    try:
        ...
        # Вычисляем площади выпуклых оболочек
        area_upper = ConvexHull(upper_points[:, :2]).area
        area_lower = ConvexHull(lower_points[:, :2]).area
        
        print(f"\nПлощадь верхней поверхности: {area_upper:.2f}")
        print(f"Площадь нижней поверхности: {area_lower:.2f}")
        
        # Выбираем основную поверхность (с меньшей площадью)
        ...
        for simplex in main_tri.simplices:
            ...
            height_diff = (z_main - z_other) if area_upper < area_lower else (z_other - z_main)
